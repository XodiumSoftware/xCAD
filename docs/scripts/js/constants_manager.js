class MatrixDisplay {
    ...
    generateMatrixContent(matrix) {
        ...
        matrix.module_matrix_pos.forEach((row) => {
            ...
            row.forEach(([layout, widget, widgetIndex, margins, alignment, sizePolicy]) => {
                content += `<div class='matrix-column'>
                                <p class='module-title'>Element</p>
                                <hr>
                                <table>
                                    <tr><td>Layout:</td><td>${this.generateLayoutDropdown(layout)}</td></tr>
                                    <tr><td>Module Type (Widget):</td><td>${this.generateModuleTypeDropdown(widget)}</td></tr>
                                    <tr><td>Module Index:</td><td><input type="number" value="${widgetIndex}" min="0" /></td></tr>
                                    <tr><td>Margins:</td><td>${this.generateMarginsInputFields(margins)}</td></tr>
                                    <tr><td>Alignment:</td><td>${this.generateAlignmentDropdown(alignment)}</td></tr>
                                    <tr><td>Size Policy:</td><td>${this.generateSizePolicyDropdowns(sizePolicy)}</td></tr>
                                </table>
                            </div>`;
            });
    ...
    generateLayoutDropdown(selectedLayout) {
        return this.generateDropdown(selectedLayout, ['VBox', 'HBox', 'Grid', 'Form']);
    }

    generateModuleTypeDropdown(selectedModuleType) {
        return this.generateDropdown(selectedModuleType, [
            "Label", "Checkbox", "SpinBox", "InputField", "Button", "GraphicsView", "TableView"
        ], 'width: 100%');
    }

    generateAlignmentDropdown(selectedAlignment) {
        return this.generateDropdown(selectedAlignment, [
            "AlignLeading", "AlignLeft", "AlignRight", "AlignTrailing",
            "AlignHCenter", "AlignVCenter", "AlignJustify", "AlignAbsolute",
            "AlignHorizontalMask", "AlignTop", "AlignBottom", "AlignCenter",
            "AlignBaseline", "AlignVerticalMask"
        ]);
    }
    ...
    generateSizePolicyDropdowns(sizePolicy) {
        ...
        sizePolicy.forEach(policy => {
            dropdownsHTML += `
                ...
                "SizeMinimum", "SizeMaximum", "SizeFixed", "SizePreferred",
                "SizeExpanding", "SizeMinimumExpanding", "SizeIgnored"
            ])}
                </div>
            `;
        });
    ...
    displayMatrix(matrixIndex) {
        const selectedMatrix = this.matrices.find(matrix => matrix.index === matrixIndex);
