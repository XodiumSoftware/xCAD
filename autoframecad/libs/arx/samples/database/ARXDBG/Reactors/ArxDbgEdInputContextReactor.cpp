...
void
ArxDbgEdInputContextReactor::beginGetColor(const AcCmColor * dfault,
					const TCHAR* promptString,
					int initGetFlags,
					const TCHAR* pKeywords)
{
    ...
    if (printDetails()) {
        // Todo: find a way to print AcCmColor
        const int nDef = dfault ? dfault->colorIndex() : 0;
        printDefaultInt(dfault ? &nDef : nullptr);
		printPrompt(promptString);
		printInitGetFlags(initGetFlags);
		printKeywords(pKeywords);
	}
					...
					const TCHAR*& pKeyword)
{
	...
	if (printDetails()) {
		...
		printReturnStatus(returnStatus);
        // Todo: find a way to print AcCmColor
        printValue(_T("COLOR"), ArxDbgUtils::colorToStr(retValue.colorIndex(), str, true));
		printKeywordPicked(pKeyword);
	}
}
