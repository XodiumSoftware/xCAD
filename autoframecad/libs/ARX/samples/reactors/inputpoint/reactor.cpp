...
void
AsdkInputContextReactor::beginGetColor(const AcCmColor * dfault,
                               const TCHAR* promptString,
                               int   initGetFlags,
                               const TCHAR* pKeywords)
{
    acutPrintf( _T("\nbeginGetColor:") );
    // We could also print getRGBM() instead of colorIndex().
    // When the color is not ACI, colorIndex() only finds the nearest ACI to that rgb.
    if ( dfault )
        acutPrintf( _T("\ndefault: %d"), dfault->colorIndex() );
    if ( promptString )
        acutPrintf( _T("\nprompt: \"%s\""), promptString );
    acutPrintf( _T("\ninitget %d"), initGetFlags );
    if ( pKeywords )
                             ...
                             const TCHAR*& pKeyword)
{
    // We could also print getRGBM() instead of colorIndex().
    // When the color is not ACI, colorIndex() only finds the nearest ACI to that rgb.
    acutPrintf( _T("\nendGetColor:\nstatus: %d\ninteger: %d"), returnStatus, retValue.colorIndex() );
    if ( pKeyword )
        acutPrintf( _T("\nkeyword: %s"), pKeyword );
    acutPrintf( _T("\n") );
}
