...
public:
    ...
    // since V23.2.04
    // get/set the name string of the space object
    static BimApi::ResultStatus getSpaceName(const AcDbObjectId& spaceId, AcString& spaceName);
    static BimApi::ResultStatus setSpaceName(const AcDbObjectId& spaceId, const ACHAR* spaceName);

    // get/set the BrxBimSpace object assigned to specified 'entityId'
    static BimApi::ResultStatus getAssignedSpace(const AcDbObjectId& entityId, AcDbObjectId& spaceId, AcString& spaceName);
    static BimApi::ResultStatus setAssignedSpace(const AcDbObjectId& entityId, const AcDbObjectId& spaceId);
    static BimApi::ResultStatus setAssignedSpace(const AcDbObjectId& entityId, const ACHAR* spaceName);

    // get all entities assigned to the specified 'spaceId' space object
    static BimApi::ResultStatus getAssignedEntities(AcDbObjectIdArray& entityIds, const AcDbObjectId& spaceId);
    static BimApi::ResultStatus getAssignedEntities(AcDbObjectIdArray& entityIds, const ACHAR* spaceName, const AcDbDatabase* pDb = nullptr);
};
...
public:
    ...
    BrxBimProfile& operator=(const BrxBimProfile& other);

    // allows to switch this BrxBimProfile object to use a different entity
    bool setFromId(const AcDbObjectId& id);

    // returns 'true' if the associated entity is of 'linear' AcDb3dSolid type
    bool isValid() const;

    AcString getShape() const;
    AcString getName() const;
    AcString getSize() const;
    AcString getStandard() const;
    AcString getDescription() const;
    ...
    BimApi::ResultStatus getProfileCurves(AcArray<AcGeCurve3dPtrArray>& profileLoops, 
    ...
    BimApi::ResultStatus applyProfileTo(const AcDbObjectId& id, double rotationAngle = 0.0, bool keepClippings = true);
    ...
    BimApi::ResultStatus saveProfile(const AcDbDatabase* db = nullptr);
    ...
    static AcString getSize(const BrxBimProfile& profile);
...
public:
    ...
    BrxBimLinearGeometry& operator=(const BrxBimLinearGeometry& other);

    // allows to switch this BrxBimLinearGeometry object to use a different entity
    bool setId(const AcDbObjectId& id);

    // allows to retrieve the associated AcDbObjectId
    const AcDbObjectId& getId() const { return m_id; };

    // returns 'true' if the associated entity is of 'linear' AcDb3dSolid type
    bool isValid() const;

    // returns the 'Axis' curve of the 'linear' AcDb3dSolid
    BimApi::ResultStatus getAxis(AcGeCurve3dPtr& axis) const;

    // returns the 'Extrusion Path' curve of the 'linear' AcDb3dSolid
    BimApi::ResultStatus getExtrusionPath(AcGeCurve3dPtr& path) const;
    ...
    BimApi::ResultStatus getProfile(AcArray<AcGeCurve3dPtrArray>& profile) const;

    // returns the BrxBimProfile assigned to a linear AcDb3dSolid, if any (otherwise empty, not valid profile)
    BimApi::ResultStatus getAssignedProfile(BrxBimProfile& profile) const;

    // returns the 'Side Faces' of the 'linear' AcDb3dSolid
    BimApi::ResultStatus getSideFaces(AcDbFullSubentPathArray& sideFaces) const;

    // returns the 'Clipping Faces' of the 'linear' AcDb3dSolid
    BimApi::ResultStatus getClippingFaces(AcDbFullSubentPathArray& clippingFaces) const;

    //returns the 'Axis' Start Point of the 'linear' AcDb3dSolid
    BimApi::ResultStatus getStartPoint(AcGePoint3d& startPoint) const;
    
    //returns the 'Axis' End Point of the 'linear' AcDb3dSolid
    BimApi::ResultStatus getEndPoint(AcGePoint3d& endPoint) const;
...
public:
    ...
    // adds the specified 'bimType' BIM Type (as global name) to the specified PropertySet
...
//----------------- BIM Attribute Set Functions -----------------

class BRX_IMPORTEXPORT BrxBimAttributeSet
{
public:
    BrxBimAttributeSet() {};
    virtual ~BrxBimAttributeSet() {};

public:
    //------------- AttributeSet related Functions ----------------

    // returns the list of available BIM AttributeSets (as mapping <name,label>)) in the given NameSpace,
    // for the specified database;
    // if no nameSpace is specified, the "User" namespace is used (nameSpace is case-insensitive);
    // if no database is specified, the current drawing database is used;
    static BimApi::ResultStatus listAttributeSets(BimPropertiesMap& attributeSets, 
                                                  const ACHAR* szNameSpace = nullptr, AcDbDatabase* pDb = nullptr);

    // verifies whether the given AttributeSet name (or label) is available in the given NameSpace,
    // for the specified database;
    // AttributeSet name is case-insensitive;
    // if no nameSpace is specified, the "User" namespace is used (nameSpace is case-insensitive);
    // if no database is specified, the current drawing database is used;
    static BimApi::ResultStatus hasAttributeSet(const ACHAR* szNameOrLabel, 
                                                const ACHAR* szNameSpace = nullptr, AcDbDatabase* pDb = nullptr);

    // returns the parameters of the given AttributeSet name (or label), of the given NameSpace,
    // for the specified database;
    // AttributeSet name is case-insensitive;
    // if no nameSpace is specified, the "User" namespace is used (nameSpace is case-insensitive)
    // if no database is specified, the current drawing database is used;
    static BimApi::ResultStatus getParameters(bool& visible, const ACHAR* szNameOrLabel, const ACHAR* szNameSpace = nullptr, AcDbDatabase* pDb = nullptr);

    // defines the parameters of the given AttributeSet name (or label), of the given NameSpace,
    // for the specified database;
    // AttributeSet name is case-insensitive;
    // if no nameSpace is specified, the "User" namespace is used (nameSpace is case-insensitive);
    // if no database is specified, the current drawing database is used;
    static BimApi::ResultStatus setParameters(bool visible, const ACHAR* szNameOrLabel, const ACHAR* szNameSpace = nullptr, AcDbDatabase* pDb = nullptr);

    // creates a new AttributeSet with the given name and (optional) label, for the given NameSpace,
    // for the specified database;
    // if label is not specified, the PropertySet name is used as label;
    // if no nameSpace is specified, the "User" namespace is used (nameSpace is case-insensitive);
    // if no database is specified, the current drawing database is used;
    static BimApi::ResultStatus createAttributeSet(const ACHAR* szName, const ACHAR* szLabel = nullptr,
                                                   const ACHAR* szNameSpace = nullptr,
                                                   bool visible = true,
                                                   AcDbDatabase* pDb = nullptr);

    // deletes the AttributeSet given by name (or label) in the specified database, case-insensitive;
    // if no nameSpace is specified, the "User" namespace is used (nameSpace is case-insensitive);
    // if no database is specified, the current drawing database is used;
    // NOTE : all Properties contained in the AttributeSet are also removed
    static BimApi::ResultStatus deleteAttributeSet(const ACHAR* szNameOrLabel, 
                                                   const ACHAR* szNameSpace = nullptr, AcDbDatabase* pDb = nullptr);

    // returns the 'Label' for the specified AttributeSet;
    // if no database is specified, the current drawing database is used;
    static BimApi::ResultStatus getLabel(AcString& label, const ACHAR* szAttributeSetName, AcDbDatabase* pDb = nullptr);

    // assigns the 'Label' for the specified AttributeSet;
    // if no database is specified, the current drawing database is used;
    static BimApi::ResultStatus setLabel(const ACHAR* szAttributeSetName, const ACHAR* szAttributeSetLabel, AcDbDatabase* pDb = nullptr);

    //------------- AttributeSet Properties related Functions ----------------

    // returns the list of available Properties (as std::vector<BimPropertyInfo>) for the given AttributeSet,
    // for the specified database;
    // if no database is specified, the current drawing database is used;
    static BimApi::ResultStatus listProperties(BimPropertiesList& properties, const ACHAR* szAttributeSetName, AcDbDatabase* pDb = nullptr);

    // verifies whether the classified BIM entity has a given Property (by Name or Label) in the specified AttributeSet;
    // AttributeSet name is case-insensitive;
    // Property name is case-insensitive;
    // NOTE : all available BIM NameSpaces are automatically searched for the specified AttributeSet
    static BimApi::ResultStatus hasProperty(const AcDbObjectId& id, const ACHAR* szAttributeSetName, const ACHAR* szPropertyName);

    // verifies whether the specified Property exists inside the specified AttributeSet;
    // if no database is specified, the current drawing database is used;
    static BimApi::ResultStatus hasProperty(const ACHAR* szAttributeSetName, const ACHAR* szPropertyName, AcDbDatabase* pDb = nullptr);

    // returns the value assigned to given Property (by Name or Label) in the specified AttributeSet, for the
    // classified BIM entity;
    // AttributeSet name is case-insensitive;
    // Property name is case-insensitive;
    // NOTE : all available BIM NameSpaces are automatically searched for the specified AttributeSet
    static BimApi::ResultStatus getProperty(const AcDbObjectId& id, const ACHAR* szAttributeSetName, const ACHAR* szPropertyName, AcValue& value);

    // assigns the value to the given Property (by Name or Label) in the specified AttributeSet, for the
    // classified BIM entity;
    // if 'autoCreateProperty' is 'true', a missing Property is automatically created (as 'visible')
    // AttributeSet name is case-insensitive;
    // Property name is case-insensitive;
    // NOTE : all available BIM NameSpaces are automatically searched for the specified AttributeSet
    static BimApi::ResultStatus setProperty(const AcDbObjectId& id, const ACHAR* szAttributeSetName, const ACHAR* szPropertyName, 
                                            const AcValue& value, bool autoCreateProperty = false);

    // create a new Property inside the specified AttributeSet, with all appropriate parameters;
    // if no database is specified, the current drawing database is used;
    static BimApi::ResultStatus createProperty(const ACHAR* szAttributeSetName, const ACHAR* szPropertyName, const ACHAR* szPropertyLabel = nullptr,
                                               const ACHAR* szPropertyDesc = nullptr, bool visible = true,
                                               AcDbDatabase* pDb = nullptr);

    // removes the given Property (by Name or Label) in the specified AttributeSet (by Name or Label),
    // from the classified BIM entity;
    // AttributeSet name is case-insensitive;
    // Property name is case-insensitive;
    // NOTE : all available BIM NameSpaces are automatically searched for the specified AttributeSet
    static BimApi::ResultStatus deleteProperty(const AcDbObjectId& id, const ACHAR* szAttributeSetName, const ACHAR* szPropertyName);

    // removes the given Property (by Name or Label) from specified AttributeSet (by Name or Label)
    // from specified database; if no database is specified, the current drawing database is used;
    // AttributeSet name is case-insensitive;
    // Property name is case-insensitive;
    // NOTE : all available BIM NameSpaces are automatically searched for the specified AttributeSet
    static BimApi::ResultStatus deleteProperty(const ACHAR* szAttributeSetName, const ACHAR* szPropertyName, AcDbDatabase* pDb = nullptr);

    // retrieves all defining parameters for the specified Property inside the specified AttributeSet;
    // if no database is specified, the current drawing database is used;
    static BimApi::ResultStatus getPropertyParameters(const ACHAR* szAttributeSetName, const ACHAR* szPropertyName, AcString& label, 
                                                      AcString& description, bool& visible,
                                                      AcDbDatabase* pDb = nullptr);

    // assigns all defining parameters for the specified Property inside the specified AttributeSet;
    // if no database is specified, the current drawing database is used;
    static BimApi::ResultStatus setPropertyParameters(const ACHAR* szAttributeSetName, const ACHAR* szPropertyName, const ACHAR* szLabel, 
                                                      const ACHAR* szDescription, bool visible,
                                                      AcDbDatabase* pDb = nullptr);

    //-------- support for 'BIM Object Types' associated with an AttributeSet

    // returns a map of 'global' BIM Type names and their associated 'localised' names
    typedef std::map<AcString, AcString>  BimTypeNames;
    static BimApi::ResultStatus listAllBimTypes(BimTypeNames& bimTypes);

    // returns the array of 'global' BIM Type names used by the specified AttributeSet (by Name or Label)
    static BimApi::ResultStatus listBimTypes(AcStringArray& bimTypes, const ACHAR* szAttributeSetName);

    // returns whether the specified AttributeSet uses the 'bimType' BIM Type (as global name)
    static BimApi::ResultStatus hasBimType(const ACHAR* szAttributeSetName, const ACHAR* szBimType);

    // adds the specified 'bimType' BIM Type (as global name) to the specified AttributeSet 
    static BimApi::ResultStatus addBimType(const ACHAR* szAttributeSetName, const ACHAR* szBimType);

    // removes the specified 'bimType' BIM Type (as global name) from the specified AttributeSet
    static BimApi::ResultStatus delBimType(const ACHAR* szAttributeSetName, const ACHAR* szBimType);
};
